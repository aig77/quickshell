...
QtObject {
  ...
  Component.onCompleted: {
    try {
      ...
      // Relative path: components/ -> quickshell/ -> .config/ -> home/ -> .cache/
      // Avoids hardcoding the username or home directory.
      xhr.open("GET", Qt.resolvedUrl("../../../.cache/stylix/colors.json"), false)
      xhr.send()
      if (xhr.responseText.length > 0)
        _p = JSON.parse(xhr.responseText)
    } catch(e) {
      console.warn("Colors: ~/.cache/stylix/colors.json not found, using Catppuccin fallback")
    }
  }
}
