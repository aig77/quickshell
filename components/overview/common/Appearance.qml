...
Singleton {
    ...
    property QtObject m3colors: {
        if (colorSource === "matugen" && matugenColorsLoaded)
            return matugenColors;
    ...
    property bool matugenColorsLoaded: false

    // Matugen colors populated from config.json via loadMatugenColors().
    // Initialized to defaultColors; overwritten when Config loads.
    ...
    function loadMatugenColors() {
        // Read colors from the already-loaded config (Config.qml uses Process + $HOME,
        // avoiding QML relative URL resolution issues across symlink chains).
        const raw = Common.Config.userOptions?.appearance?.matugenColors;
        if (!raw) {
            console.warn("Appearance: matugenColors not found in config.json, using default colors");
            return;
        }
        const keys = [
            ...
            "m3background", "m3onBackground", "m3surface",
            "m3surfaceContainerLow", "m3surfaceContainer", "m3surfaceContainerHigh", "m3surfaceContainerHighest",
            "m3onSurface", "m3surfaceVariant", "m3onSurfaceVariant",
            "m3inverseSurface", "m3inverseOnSurface",
            "m3outline", "m3outlineVariant", "m3shadow"
        ];
        for (const key of keys) {
            if (raw[key]) matugenColors[key] = raw[key];
        }
        root.matugenColorsLoaded = true;
    }
